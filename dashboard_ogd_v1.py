...
# OGD Parquet URL (kompletter Datensatz)
...
def load_all_data_from_ogd():
    """Lädt den kompletten Datensatz als Parquet vom OGD Portal."""
    try:
        response = requests.get(OGD_PARQUET_URL, timeout=120, verify=False)
        response.raise_for_status()
        return pd.read_parquet(BytesIO(response.content))
    except requests.exceptions.RequestException as e:
        st.warning(f"Fehler beim Laden der Daten: {e}")
        return None
...
def load_data_for_years(selected_years):
    """
    Lädt den kompletten Datensatz und filtert nach ausgewählten Jahren.
    """
    data = load_all_data_from_ogd()
    
    if data is None or data.empty:
        return None
    
    # Datum parsen (falls nicht bereits datetime)
    if not pd.api.types.is_datetime64_any_dtype(data['Datum']):
        data['Datum'] = pd.to_datetime(data['Datum'], format='ISO8601')
    
    # Jahr extrahieren und nach ausgewählten Jahren filtern
    data['Jahr'] = data['Datum'].dt.year
    data = data[data['Jahr'].isin(selected_years)].copy()
